...
public class FullCalendarView {
	...
	public FullCalendarView(YearMonth yearMonth) {
		...
		for (Text txt : dayNames) {
			...
			ap.setStyle("-fx-background: #55555s5;");// TRY IF work
	...
	public void populateCalendar(YearMonth yearMonth) {
		...
		for (AnchorPaneNode ap : allCalendarDays) {
			...
			ap.getChildren().add(txt);
			if (Math.random() < 0.25) {//Bedinung ändern (Bedingung Prozentuell machen -> nur 1 Mal die If am)
				//Switch case mit einzelnen Ansichten (Zimmer, Sauna, Kegelbahn usw...)
				ap.setBackground(
						new Background(new BackgroundFill(Paint.valueOf("YELLOW"), CornerRadii.EMPTY, Insets.EMPTY)));
				...
				ap.setBackground(
						new Background(new BackgroundFill(Paint.valueOf("ORANGE"), CornerRadii.EMPTY, Insets.EMPTY)));
			}
			else {
				ap.setBackground(
